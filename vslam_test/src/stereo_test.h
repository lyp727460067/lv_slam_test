...
class VTrack {
 public:
  std::map<int, KeyPoint> OpticalTracking(cv::Mat init_pose,
                                          FrameImag* pre_frame,
                                          FrameImag* cur_frame) {
  ...
  std::map<int, KeyPoint> RemoveOutLiner(
      const std::map<int, KeyPoint>& pre_points,
  ...
  std::vector<cv::Point2f> KeyPointToCvPoints(
      const std::map<int, KeyPoint>& key_points) {
  ...
  std::map<int, KeyPoint> ExtraFeature(cv::Mat&& mask, const cv::Mat& img,
                                       int feature_num) {
    if(feature_num==0)return std::map<int, KeyPoint>();
...
class StereoTrack {
 ...
 private:
  ...
  void Init(const cv::Mat& left_cam, const cv::Mat& right_cam) {
    ...
    // cv::cornerSubPix(left_cam, left_feats, cv::Size(5, 5), cv::Size(-1,
    // -1),criteria);
    ...
    for (int i = 0; i < right_feats.size(); i++) {
      if (status[i]) {
        ...
        cv::circle(ViewMat, right_feats[i] + cv::Point2f(left_cam.cols, 0), 2,
                   cv::Scalar(0, 255, 0));
      }
    }
  ...
  void Visualize(const cv::Mat& img, const std::vector<cv::Point2f>& cur_feats,
                 std::vector<cv::Point2f> pre_feats,
                 std::vector<uint8_t>& status) {
    cv::Mat img_feat;
  ...
  bool TrackKeyFrame(const cv::Mat& key_img, const cv::Mat& curr_img) {
    ...
    if (ret) {
      ...
      pose_.block<3, 1>(0, 3) =
          k_pose_.block<3, 1>(0, 3) + k_pose_.block<3, 3>(0, 0) * dt;
