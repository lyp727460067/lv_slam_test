...
void stereo_tracker(const std::pair<cv::Mat, cv::Mat>& stereo_imag,
                    EkfTest* ekf) {
  ekf->PushData(stereo_img.first);
  Eigen::Matrix4d pose = ekf->updata();
  PubTf(pose);
...
void OdometryCallback(const nav_msgs::Odometry::ConstPtr odom,EkfTest*ekf)
{
	OdomData odom_data;
	odom_data.theta =  2*acos(odom->pose.pose.orientation.w);
	odom_data.tranlation.x() =  odom->pose.pose.position.x;
	odom_data.tranlation.y() =  odom->pose.pose.position.y;
	odom_data.time   =  odom->header.stamp.toNSec();	
	ekf->PushData(odom_data);
}
void ImuCallback(const sensor_msgs::ImuConstPtr imu, EkfTest* efk) {
  ImuData imu_data;
  imu_data.gry = {imu->angular_velocity.x, imu->angular_velocity.y,
                  imu->angular_velocity.z};
  imu_data.acc = {imu->linear_acceleration.x, imu->linear_acceleration.y,
                  imu->linear_acceleration.z};
  efk->PushData(imu_data);
}
